...
from .controls import WELL_STATUSES, WELL_TYPES, WELL_COLORS,wellspublic_query,query1,db_info
from scipy.constants.constants import gas_constant
from .new_points import get_dt_set
server = Flask(__name__)
server.secret_key = os.environ.get('secret_key', 'secret')

#app1 = dash.Dash(__name__, server=server, url_base_pathname='/dash/gallery/new-york-oil-and-gas/', csrf_protect=False)  # noqa: E501
#app1.css.append_css({'external_url': 'https://cdn.rawgit.com/plotly/dash-app-stylesheets/2d266c578d2a6e8850ebce48fdb52759b2aef506/stylesheet-oil-and-gas.css'})  # noqa: E501
'''
...
df = pd.read_sql_query(wellspublic_query,db)
...
points = get_dt_set()#pd.read_sql_query(query1,db)
#print points
...
layout = html.Div(
    [
        ...
        html.Div(
            [
                html.H1(
                    'Texas Oil and Gas - Production Overview'
                ),
            ],
            className='row'
        ),
...
def filter_dataframe(df, well_statuses, well_types, year_slider):
    well_stat = []
    well_ty = []
    if not isinstance(well_statuses,list):
        well_stat.append(well_statuses)
        well_statuses = well_stat
    if not isinstance(well_types,list):
        well_ty.append(well_types)
        well_types = well_ty
...
def update_year_slider(count_graph_selected):
    ...
    if count_graph_selected is None:
        ...
        #print count_graph_selected['points']
        nums = []
